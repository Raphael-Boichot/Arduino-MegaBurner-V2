...
I've made this project because I had an urgent need to read / write the three chips supported here for a [Neo Geo bootleg project](https://github.com/Raphael-Boichot/Neo-Geo-Sengoku-2-Red-Blood). Using a [Sanni Cartreader](https://github.com/sanni/cartreader) was an option, this is a great project, but not for me right now: too complicated / too expensive for just burning a chip.

This project so originates from the more straightforward [maximaas/MegaBurner](https://github.com/maximaas/MegaBurner), a Java + SWT desktop application built to flash specifically the MX29L3211 chip for SNES/SFC cartridge reproductions, talking to an Arduino Mega 2560 over serial. The original Java / Arduino codebase, despite being great and functional like a real EEPROM flasher, has a hell of old dependencies (the project was made in 2017) and is not portable at all due to the huge libraries required for the GUI. After compiling the working Java code, I came to the conclusion that I needed something probably less fancy but much easier to debug. So:
...
Next is my prototype, perfectly working despite the mess of wires.
